...
public class Spline2D {
    ...
    /// <summary>
    /// Converts this Catmull-Rom spline into a very similar (but not exact) list of 
    /// Bezier spline nodes.
    /// </summary>
    /// <returns></returns>
    ...
    {
        Recalculate(true);
        Vector2[] points2D = points.ToArray();
        Vector3[] bezierPoints = new Vector3[points2D.Length];
        for (int i = 0; i < points2D.Length; i++)
        {
            bezierPoints[i] = (Vector3) points2D[i];
        }
        ...
        {
            position = bezierPoints[i];
            distance = (bezierPoints[i + 1] - bezierPoints[i - 1]).magnitude;
            // rightTangent = Derivative(i, 0.01f).normalized * distance;
            rightTangent = tangents[i].normalized * distance;
    ...
    /// <summary>
    /// Returns a list of Bezier nodes that are placed at regular intervals along the curve.
    /// </summary>
    /// <param name="interval"></param>
    /// <returns></returns>
    public BezierSplineNode[] GetRegularIntervalBezierNodes(float interval)
    {
        List<BezierSplineNode> bezierNodes = new List<BezierSplineNode>();
        Vector3 position;
        ...
        Vector3 rightTangent;
        Vector3 leftTangent;
        float curveLength = DistanceAtPoint(points.Count - 2);
        
        for (float dist = DistanceAtPoint(1); dist < curveLength; dist += interval)
        {
            // Debug.Log("distance = " + distance + ", ")
            position = InterpolateDistance(dist);
            rightTangent = DerivativeDistance(dist);
            leftTangent = Quaternion.Euler(0, 0, 180) * rightTangent;
            bezierNodes.Add(new BezierSplineNode(position, rightTangent, leftTangent));	
        }
